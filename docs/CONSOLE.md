...
- **Nothing is read before the adapter exists.** The static shell satisfies selectors with markup
  ...
  emptied — see §1.)
- **The rail tile's click is a write**, so that leg is local-box only. So is `--approval`, which
  arms the box's own review mode, forces a real approval and presses its buttons; it is refused
  outright against a `--url`.
...
| `node --test tests/console-approval-card.test.mjs` | the approval card: four pill states, the 400-character elision and its count, the stripped location clause, the conditional rule paragraph and Always-allow button, the settled branches keeping the command, and the order of the two calls behind Always allow (§8) |
...
**That was true of CONSOLE-4 and it is NOT true of console polish 3.** CONSOLE-4 was relay-only by
construction — everything in it was `ui/`, tests, scripts and docs — and its ship was a relay sync
and one relay restart. Console polish 3 adds one sentence to the standing persona
(`source/host/runner/standing-persona.ts`), which is a host change, so that wave's ship is a bundle
swap in exactly two boxes with the post-swap watch, and the `git diff -- source/ deploy/` assertion
above belongs to CONSOLE-4's ship rather than to every ship of this file. The rule the assertion was
protecting still stands: if a wave that believed itself relay-only turns out to touch `source/` or
`deploy/`, stop and re-scope, because a host swap is a different ship with a different risk.

Nothing in §8 is part of that host change. The approval card is `ui/` alone.
...
## 8. The approval card (COMMAND-CARD-1)
