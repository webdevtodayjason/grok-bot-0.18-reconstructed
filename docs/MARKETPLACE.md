...
| --- | --- | --- |
| not installed | the plugin's connector name is not in `connectors.json` (for a shell-tool plugin: the tool is not installed in the box) | the connectors file / the shell-tool install probe |
...
- A **shell tool** (CodeRabbit CLI, TinyFish CLI) is not an MCP server: it is a command-line program
  ...
  `tools/list`.
- **Custom MCP server** is the catalog entry for everything the catalog does not know. Its Add opens
  the connector editor that already exists — name, command, arguments, environment variable names —
  so a server with no card is still added from the same place.

**Uninstall** is on the plugin page. It removes the entry from `connectors.json` and re-reads the
file, and it offers to clear that plugin's stored secrets (`deleteConnectorSecret`) as a separate,
...
2. **Press Import Bot.** The console calls `createAgent` with the bot's name and its description,
   carrying the bot's instructions as the agent's persona — on this host the profile's `description`
   *is* the persona field, which is what the agent-side CreateAgent tool means by "an optional
   persona/description" (`source/host/runner/tools/sand-agent-management-tools.ts`). There is no
   separate system-prompt field on an agent to put it in.
...
| `install` | for a connector, the exact `{ command, args, env }` entry; for a shell tool, the shell-tool id |
...
**The install block is one object, in one place.** The console's connector presets already carry
these entries character for character out of the reports in `docs/connectors/`, and
`tests/connector-preset-catalog.test.mjs` fails if a preset and its report drift apart. The catalog
holds the same rule: a connector entry exists once, and both the preset row and the catalog card fill
from it. Two copies of the same `args` array is how a header ends up quoted in one place and split in
the other, which is exactly the bug the quoting rule in the connector editor exists to prevent. The
