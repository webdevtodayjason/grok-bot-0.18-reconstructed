...
(ONBOARD-5, 2026-09-12). It is still a bearer credential in a URL -- whoever holds it is signed in as
that person -- so it is sent the way a password is sent; what changed is that its id is written down
...
before 2026-09-12 is refused now**, because its id was never recorded. Replies come back to
`mail.welcome.replyTo`, which defaults to
