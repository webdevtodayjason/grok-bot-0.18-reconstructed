...
| MARKET-1 | The Plugins panel lists providers, connectors, shell tools and listeners as one flat nav; the operator wants the original's marketplace: a Plugins tab (search, categories, Featured, cards with Add/Added, a plugin page with Accounts to authenticate, its Connectors, Uninstall, View source) and a Bots tab (importable bot templates with Instructions, Skills, Integrations, Import Bot) | A local catalog (plugins and bots) shipped in the bundle and served by the gateway; the Marketplace panel in the console with both tabs; Add installs the preset through the existing connector path and opens the plugin page; the credential card is the Accounts row ("Needs auth" until stored) | M |
| PROVIDERS-1 | Providers (subscriptions, endpoint keys) are shown beside connectors in the Plugins panel; the operator wants them only under Settings | Providers move to Settings → Inference as their own section, same cards; the chat listeners (Slack listener) move to Settings too; the marketplace holds plugins and bots only | S |
| BOTS-1 | No bot templates: every agent starts blank | A bots catalog (name, creator, tile colour, description, instructions, skills as SKILL.md, integrations as plugin ids), a Bots tab with Featured and categories, a bot page, and Import Bot = createAgent with the instructions plus importAgentWorkflowText per skill, with the missing integrations offered as Add | M |
| PLUGINTOOLS-1 | The agents' SearchPlugins, GetPlugin, InstallPlugin and UninstallPlugin read Cursor's marketplace, which this box cannot reach, so the catalog is always empty and the tools are dead ends | The same local catalog behind the tools: SearchPlugins lists it, GetPlugin describes a plugin and its credential fields, InstallPlugin writes the preset entry and tells the model which card the operator must fill, UninstallPlugin removes it; a `verify-connector-plane --plugin-tools` arm | S |
