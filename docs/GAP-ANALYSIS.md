...
| GATE-4 | A gate killed by its `timeout` (SIGTERM) never reaches its finally: five `verify-cp-*` probe agents were left on the roster this afternoon, and a planted routine or a gateprobe connector can be left the same way | Every verify script installs a SIGTERM and SIGINT handler that runs the same cleanup its finally runs, then exits 143; `on-box.sh` keeps using SIGTERM. Sweep: the five leaked agents deleted once the current dashboard run is over | S |
