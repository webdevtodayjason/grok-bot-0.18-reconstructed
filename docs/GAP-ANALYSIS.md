...
| LOOP-1 | On the R750 with qwen3.8-max the agent kept sending sign-off messages, one per runner step, about fifty in ten minutes, until the container was restarted; each was its own attempt with no tool but SendMessage. Nothing in the runner ends a turn whose only activity is the model talking to itself | A runner guard: after N consecutive steps that produced only SendMessage and no other tool, end the turn and say so in the transcript; a gate that drives a model into the pattern with a stub and asserts the cap | M |
