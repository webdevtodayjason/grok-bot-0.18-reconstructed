...
| CONNECT-2 | A connector that cannot connect (the TinyFish bridge waiting on its OAuth sign-in) makes every MCP list call wait on its sixty-second connection timeout, and the console boots on those lists, so the page took over a minute to paint; removing the entry did not clear it until a box restart, because the host keeps a loaded server | List calls answer from the last known state and never block on a connecting server; a server in error is reported, not awaited; removal takes effect on refresh | S |
| LOOP-2 | SendMessage's MAX_SENDS_PER_TURN of 20 and its duplicate suppressor are per step, not per turn (30 sends in one turn measured before the cap existed) | Count per turn | S |
...
| AUTOMATION-3 | A scheduled run that fails no longer raises a tray error (the manual path did), and no surface in the console shows a failed scheduled run | A run-history read on the routine card, or a transcript notice on failure | S |
| TOOLS-18 | The toolset and the prompt each ask the local-exec bridge whether a machine is connected, so a heartbeat lapse between the two reads can send a prompt that teaches five tools the wire withholds; `SAND_LOCAL_MACHINE=1` pins the connected world even with no daemon | One read per turn shared by both, and the pin honoured only while a daemon answers | S |
| TEACH-4 | A recording that runs to the ten-minute cap is saved with no operator note, and the dialog's hint claims the note always reaches the learning turn | Send the dialog's current note text with the cap save, or say in the hint that the cap saves without it | S |
...
**Wave U3 — what the first public hour taught (LOOP-1 and CONNECT-1 landed; the rest is the RESIDUALS stage).** LOOP-1 with a stub-driven gate,
