...
| MR-31 | A dashboard run failed on the roster's Hidden group: a `control-shelf` workspace chip intercepted the click on `[data-roster-hidden] > summary` (seen once at 13:44, not at 13:57, so it depends on what the shelf holds) | Find the shelf state that overlaps the roster's Hidden summary and give the summary its own stacking or the shelf a bound; a gate check that clicks Hidden with a populated shelf | S |
