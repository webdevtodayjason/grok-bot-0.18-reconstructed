...
| Id | Finding | Next | Size |
|---|---|---|---|
| JOBBUS-1 | Nothing on this instance accepts work from another machine. The Chief of Staff can only reach an agent the way a person does, through the console or a chat listener, and it gets back prose it has to believe | The gateway half in `source/host/extensions/job-bus/`: the store on the box data volume (`job-bus/jobs.json` written atomically, the last 500 terminal jobs kept; `job-bus/audit.jsonl` append-only), the type allowlist, idempotency, `jobBusHealth` / `jobBusCreate` / `jobBusGet` / `jobBusCancel` / `jobBusArtifacts` / `jobBusList` registered in `gateway-protocol.ts`, and one in-host worker that sends §6's prompt into the worker agent's own conversation and attests the reply with `readAgentEvidence`. `SAND_JOB_BUS_WORKERS`, `SAND_JOB_BUS_TIMEOUT_MIN`, `SAND_JOB_BUS_ENABLED` | L |
| JOBBUS-2 | The relay's only door is the console session, so there is no way to let a machine in without also letting it hold the whole gateway surface | The `/v1` edge in `ui/server.mjs`, handled BEFORE the console auth gate and never accepting a console cookie: bearer on every route including health, `401` with `www-authenticate`, `413` over 64 KB, `429` over 120 a minute, `404`/`405`, the secret-shaped payload refusal, header-over-body idempotency, `submitter: "cos"` added by the relay. The token resolves env, then `<profile>/job-bus.json` at 0600, then 503, compared in constant time. The four console routes that generate, set and clear it | M |
| JOBBUS-3 | A bus with no surface is a bus nobody can see: no way to mint a token, no way to know whether one is set or where it came from, no way to see what the CoS has been asking for or which job is stuck on a human | Settings → Job bus: the configured state and its source, the base URL, Generate / Set / Clear with the generated token shown once beside its warning, a curl example carrying a placeholder, the worker mapping as editable type-to-agent rows, and the jobs table refreshed on the host's `job-bus` event. Gates: the offline dashboard arm renders the card unconfigured and configured, `scripts/verify-job-bus.mjs` drives `/v1` end to end through a relay it starts itself, `verify-deploy.mjs --job-token` proves the door. Deploy: `TITAN_JOB_TOKEN` through the Coolify compose and the R750 relay, with the operator steps in both READMEs and the runbook | M |
| JOBBUS-4 | Nothing above is proven on Jason's own instance: the gates run on this Mac and the box here, and the CoS has never called the R750 | Ship it to the R750, set the token (Coolify field or the console's Generate), make sure Scribe exists with a GitHub credential, then the §8 smoke from the CoS box: health, one `health.ping`, the read back. Open until he runs it | S |
| JOBBUS-5 | The three pieces above were built to `docs/JOB-BUS.md` sections 1 to 9. Section 10, the hardening pass, was written after them and wins wherever the two disagree, so every line of it is unbuilt: the payload is not validated to 10.1's regexes and repo allowlist and is still interpolated into the prompt rather than fenced as a data block; the worker prompt goes into the mapped agent's own conversation instead of 10.2's per-job clone with stripped connectors; 10.3's persist-before-dispatch, `queueTimeoutMin`, `maxOpen` and `retired` idempotency keys are absent; attestation has only 10.4's first layer, so nothing checks GitHub out of band; the audit rows are not chained by 10.5's `prev` sha; the edge answers `503` where 10.6 fixes `401`, and has neither lockout nor global bucket; and there is no `job-bus/settings.json` behind 10.7's `jobBusGetSettings` / `jobBusSetSettings`, so the bus is on by default when 10.7 says it must be off until the operator turns it on | Build section 10 in the same three lanes it splits along, host then edge then console, and amend the gates as 10.8 says. Take 10.7 (settings file, default off) and 10.6 (uniform `401`, lockout) first: those two are the difference between a bus that is closed until Jason opens it and one that is open the moment the code lands. Nothing here is proven on the R750 either, so it merges with JOBBUS-4's smoke rather than preceding it | L |
