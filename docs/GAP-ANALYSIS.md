...
| JOBBUS-5 | §10.4's second layer is the one part of the contract nothing here can exercise: the out-of-band GitHub check needs the box's own `GITHUB_TOKEN` and a real repository, so commit-exists, branch-contains, `size == bytes`, `sha256(content) == sha256` and the `PLACEHOLDER_LOAD_FROM_DISK` refusal are all code with no run behind them, and a bug in any of them fails open into `done` rather than closed. Three conditions the skeptic named also stay open after §10, by design rather than by oversight, and none of them is written down anywhere an operator reads: (a) the door is one static shared bearer, not mTLS and not a short-lived per-caller JWT, so a single leaked value is the whole bus and rotation is a person editing an environment variable; (b) the per-job clone is isolated at the CONNECTOR level and nowhere else, so it still holds shell, browser and computer tools by design, and a prompt that reaches the model can run them in the sandbox: the bus's promise is that an unreceipted claim fails, not that the worker is confined; (c) the audit chain lives on the same box volume the worker can reach, so `prev` proves no row was edited in place but not that the file was not truncated and re-chained whole, and there is no anchor off the box | Prove layer 2 on the R750 with one real `nextgen.chapter` job against `webdevtodayjason/nextgen-training`, and prove it BOTH ways: a job whose commits and files are really there reaching `done`, and a doctored result whose sha is not on the branch failing with `commit:<sha>` unsupported. That merges with JOBBUS-4's smoke rather than preceding it. For the three residual conditions, first write them into `docs/OPERATOR-RUNBOOK.md` beside the setup steps so the operator is choosing them rather than inheriting them, then take one decision with Jason per condition: a per-caller credential (mTLS or a signed short-lived token) for (a); a tool-stripped clone, which is a host change and not a settings one, for (b); the audit head mirrored off the box, or the chain signed with a key the worker cannot reach, for (c). None is a v1 blocker; all three are v2 candidates | M |
## 1. Executive summary
