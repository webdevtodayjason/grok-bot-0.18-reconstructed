...
| CONNECT-1 | The model's AddMcpServer tool fails with "parse7 is not a function": a bundled dependency's `require_parser_sync()` returns a module object where a function is expected (an ESM/CJS interop slip inside the bundle); never exercised by a gate before Titan tried to add TinyFish | Find the module at that call site in the bundle, fix the import, and give verify-connector-plane an arm that calls AddMcpServer through a probe turn | S |
...
| DISPLAY-7 | Something brings the hidden blank agent's window back within twenty seconds of every boot, after the start-time release | Find the caller (a status read from a page, or a recovery path) and stop it asking for a blank agent's screen | S |
