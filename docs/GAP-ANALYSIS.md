...
| CONNECT-3 | TinyFish reaches the box only through OAuth (its MCP endpoint refuses `X-API-Key`, measured), which means a browser sign-in inside each box, tokens on the data volume, and a bridge that needs the sign-in re-armed after a restart until they exist; the operator would rather paste the API key | Recipe B: a small local stdio MCP bridge in the image (node, no dependencies) that speaks TinyFish's REST API (search, fetch, run-sse automation, get run) with the key read from the connector secret store, never from `connectors.json`; the console's connector card offers it as "TinyFish (API key)" beside the OAuth one; `verify-connector-plane` gains an arm with a stub REST server | M |
