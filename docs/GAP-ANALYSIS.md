...
| GATE-5 | The plain connector-plane gate cannot fit a 280 s ceiling when the box's local model starts cold: its model leg allows 300 s and glm-5.3 answers in about 240 s cold and 30 s warm, so three runs died on latency alone and a warm run took 76 s; a timeout kill then skips the cleanup (GATE-4) | Run it with `timeout 450` (done in the final sequence) or warm the model with one probe turn first; the gate itself prints how long its model leg took | S |
