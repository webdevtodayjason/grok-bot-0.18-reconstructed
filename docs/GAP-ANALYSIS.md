...
| SUB-2 · **render landed; wire proof SUB-2b in flight** | Browser tools return text only; the per-action screenshot the driver captures stays on the box | Carry `imageB64` as an image part the way the Computer adapter does (`createImageResult`) | S |
...
Twelve cheap fixes from the sweep run as their own workflow (REPORTS-1, TOOLS-12, WORKFLOW-1,
SP-3, FLAGS-2, TOOLS-16, MR-07b, DISPLAY-1b, the toolset gate's selection hole, SUB-2b, two gate
comments and GW-10's forced state); the rest are rows below.
...
| GW-16 | The takeover path (request_box_help, pending handoff, the operator sees the box) is proven nowhere; the dashboard gate forces the visible state | Drive the real path once in the gate: the host writes the pending handoff, the button appears by itself, the click clears it | S |
| SP-3 · in flight | The untrusted-data fence is the product's only tool-result control and nothing proves it reaches the model | A `spotlight` marker in the section report and the toolset gate asserting it when the flag is on | S |
| TOOLS-16 · in flight | The system prompt tells the model to use GenerateImage, which is not on the wire | Condition or drop the sentence until TOOLS-05 lands | S |
| FLAGS-2 · in flight | Two gates that decide whether shipped features run (memory dreaming, auto review) are missing from the startup table | Add both rows with their source | S |
...
| DEPLOY-1 · **decided: R750** | The product runs only on this Mac's docker box; Jason wants his own instance on the R750 (Linux docker host, always on), reached at `tb.semfreak.dev` (decided 2026-09-04; measured: Ubuntu 24.04, docker 29 usable, node 24, A40, 72 cores, 2 TB, 98 GB free on the root disk, Coolify-style containers already present), then his skills restored onto it | Wave S: an install script for a Linux docker host (box, patched bundle, relay, Machine Room behind auth on the tailnet), the recreate script generalised, a first-run checklist; then the skills restore | M |
...
| DISPLAY-1 · **landed; loader guard DISPLAY-1b in flight** | Deleted agents kept their shared-desktop window in `/home/box/.sand-window-assignments.json` (one entry sat under the key "undefined"); after enough probe agents a fresh one could not get a window and its browser child failed with "could not identify this agent's own display" | `deleteAgents` now calls the forever-box `releaseAgent`; `ensureForeverBox` refuses a call with no agent id; the assignments loader drops keys that are not agent ids; the live file was rewritten to the four live agents once | S |
