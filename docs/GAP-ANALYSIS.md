...
| MR-31 | A dashboard run failed on the roster's Hidden group: the `control-shelf` (z-index 30, spanning the width, top at y 888 at the gate's viewport) covered the click on `[data-roster-hidden] > summary`. Root cause measured by the regate: `.worker-roster` is `overflow-y: visible` and never scrolls, so at eight cards its bottom sits 5 px above the shelf and anything below that, the Hidden group first, is unclickable | Give the roster a bounded height and `overflow-y: auto` so it scrolls above the shelf; a gate check that mints enough agents to overflow, then clicks Hidden | S |
