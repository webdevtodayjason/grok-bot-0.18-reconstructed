# Upstream artifact: the teach-a-task workflow prompt, verbatim

Captured 2026-08-30: the operator recorded a teach-a-task session in the real Grok Bot and
pasted the exact prompt the host sends the agent when a teach recording lands. This is
primary-source material for Wave 1 (prompt assembly) and Wave 4 (upstream intent) — the
only upstream orchestration prompt we hold in full. Everything below the rule is verbatim.

What it proves about the architecture, in one place:
- **Skills are written with `update_state` (target "workflow", action "write")** — the
  sand-state-tool in our source is the skill-authoring surface, and skills link as
  `sand-workflow:<skill-id>`.
- A **teach-recording queue** lives at `/workspace/teach-sessions/queues/<scope>/` with
  `pending/`/`claimed/` dirs, flock-guarded claims, 12-hour leases, and stale-claim
  recovery — orchestration state as files on the box.
- A **`watchVideo` subagent** takes video file_attachments (15MB cap; lossless
  `-c copy -f segment` splits targeted at 12MB) and returns a structured play-by-play.
- Reconstruction cross-checks **browser evidence**: DevTools `/json/list` plus a *copy* of
  the Chrome History sqlite, with hard read-only rules (no `/json/new`, no websockets, no
  Page.navigate/Runtime.evaluate, no playwright, no cookie scraping) and an explicit
  "never reuse this path when running a learned skill".
- Skill-writing doctrine: parameterize inputs vs constants; stable targets (URLs, labeled
  controls) over coordinates; prefer a connector/MCP tool over UI replay; consequential
  steps are confirm-first; credentials never embedded ("assumes signed in to X"); **no
  harness mechanics in skill bodies** — "Execution and delegation are owned by the parent
  system prompt."
- Work-in-the-open is mandated even here: acknowledge via SendMessage, narrate beats.
- Scheduling = "a routine that @-mentions the workflow"; offer it only when recurring.
- Sensitive-info rules degrade gracefully: a mostly-credentials demo produces no skill.
