...
- Inline credential requests, as of 2026-09-06 (SECRET-1): an agent asks with `SendMessage type:"secret-request"` and `secret {label, description, connector, field}`; the console draws the masked card and `submitSecret {entryId, value, agentId}` answers it. `routeSecret` (widget-responses.ts) now picks between FOUR destinations in this order: the reserved connector name `shell` goes to the agent's OWN box shell environment through `tm.shellSecretSink` (writeShellEnvSecret plus the same box push setShellSecret uses, `field` is the env var name), `slack`/`github` go to the per-agent chat-channel store, any other name goes to that local stdio connector's process env, and anything unrecognized falls through to the channel file. The shell field rule is one shared predicate (`shell-tools/shell-secret-field.ts`, used by the tool schema and by `requireShellField`): UPPERCASE only, with process control, the shell's own HOME/PWD/USER/SHELL and the host's `SAND_*` refused; a refusal stores NOTHING anywhere and the agent is told the request went unanswered. The value never enters the markup, the transcript or the model's context; the ack names the variable and whether the live box took the update. Receipts: `verify-connector-plane.mjs --shell-secrets` leg (m2) drives the whole path with a probe agent and compares the store's sha256 against the submitted value, `verify-dashboard.mjs --offline` asserts the card's copy and that no input value survives a submit, `tests/secret-request-shell.test.mjs` pins the route, the refusals and the ack. That gate now uses GATE-8's three exit codes: an (m2) that the model never answers is exit 3, not a silent 0.
