...
const LEGS = ["boot", "scroll", "picker", "badge", "tile", "files", "chips"];
...
if (chosen.length === 0) {
  console.log("usage: node scripts/verify-console-polish.mjs (--boot | --scroll | --picker | --badge | --tile | --files | --chips | --all)");
...
const GATE_UA = gateUserAgent(import.meta.url);
const authHeaders = BEARER ? { authorization: `Bearer ${BEARER}` } : {};
const headers = { "user-agent": GATE_UA, ...authHeaders };
...
const RUNNER = { boot: legBoot, scroll: legScroll, picker: legPicker, badge: legBadge, tile: legTile, files: legFiles, chips: legChips };
