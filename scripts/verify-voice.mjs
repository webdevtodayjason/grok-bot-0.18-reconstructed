...
//   --leg overlay   VOICE-7: the panel over the conversation, the words being built in it, the row it
//                   becomes, and the two talk modes -- at 1440x900 and at 390x844 with a real touch
//                   hold. The footer's rect is asserted before, during and after every turn.
...
const LEGS = ["cp", "relay", "nokey", "caps", "origin", "refused", "browser", "overlay"];
const leg = (() => {
...
// ---- leg: overlay (VOICE-7) -----------------------------------------------------------------------
//
// Jason, 2026-09-10 11:09: "a semi-transparent modal over the current chat window where that is being
// built out. We see the words being created, and when it's done, that just becomes the next line ...
// Also the talk button should be either: press it and it's on, so it's a toggle, on or off; or press
// and hold to talk and let go."
//
// THE ROW THIS LEG EXISTS TO HOLD. VOICE-6's defect was the footer growing when somebody talked:
// MEASURED on this Mac in headless Chrome against static ui/machine-room, 2026-09-10, the shipped
// caption strip took .control-shelf from 1392x106 at y776 to 1392x160 with a caption and 1392x178
// with a note row at 1440x900, narrowing #composer from 600 to 568 px; and from 390x133 to 390x199
// and then 390x255 at 390x844. Width never moved at either size, so a height-only assertion would
// pass a page that still reflows sideways. This leg asserts the shelf's height AND width AND the
// composer's AND the talk button's rects, before, during and after a turn, in both modes at both
// sizes.
//
// TWO SIZES AND A REAL HOLD. The VOICE-1 browser leg runs one viewport and never sets touch, and a
// page.click() is not proof a thumb can hold a 38 px circle -- so the phone half presses through
// Input.dispatchTouchEvent, which is a real touch the browser turns into a real pointer sequence,
// and holds it for the length of the utterance.
//
// NO REAL VENDOR AND NO REAL SPOKEN TURN. The transcript events come from the stub, which speaks each
// service's own event shape. No realtime key exists on this machine and setting one over ssh would be
// the hand operation this product is being rebuilt to end.
async function legOverlay() {
  ...
  for (const view of [
    ...
    for (const mode of ["push", "always"]) {
      ...
      const set = await page.evaluate(() => window.__voice.getTalkMode());
