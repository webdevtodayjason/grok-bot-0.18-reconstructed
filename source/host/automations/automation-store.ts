...
export function parseStoredConfig(raw: string, fallbackCreatedAt: number): AutomationConfig | null {
  ...
  return { name, prompt, trigger, isEnabled: parsed.enabled !== false, heavy: parsed.heavy === true, createdAt: Math.min(authored, fallbackCreatedAt), lastRunAt: typeof parsed.lastRunAt === "number" && Number.isFinite(parsed.lastRunAt) ? parsed.lastRunAt : null, raisedNotices: parseRaisedNotices(parsed.raisedNotices) };
}
export function serializeConfig(config: AutomationConfig): string { const schedule = triggerSchedule(config.trigger); return `${JSON.stringify({ name: config.name, prompt: config.prompt, ...(schedule != null ? { schedule } : {}), ...(config.trigger.type === "cron" ? {} : { trigger: serializeStoredTrigger(config.trigger) }), enabled: config.isEnabled, ...(config.heavy ? { heavy: true } : {}), createdAt: config.createdAt, lastRunAt: config.lastRunAt, ...(config.raisedNotices.length ? { raisedNotices: config.raisedNotices } : {}) }, null, 2)}\n`; }
