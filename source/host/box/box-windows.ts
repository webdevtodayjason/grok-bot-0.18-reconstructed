...
/** The box writes one file per live seat here, named by display index; start-window binds it, stop-window removes it. */
export const SAND_BOX_WINDOW_TOKEN_DIR = "/tmp/sand-window-tokens.d";
/** DISPLAY-6: a seat whose assignment is gone leaves nothing behind but this file, so the directory is the only place an orphan shows up. */
export async function listSandWindowTokenIndexes(ctx: Context, accessor: ShellAccessor): Promise<number[]> { const result = await accessor.get(shellExecutorResource).execute(ctx, buildHostShellArgs({ command: `ls ${SAND_BOX_WINDOW_TOKEN_DIR} 2>/dev/null || true`, name: "ls", workingDirectory: "/workspace", toolCallId: "sand-window-token-scan" })); if (result.result.case !== "success") throw new SandBoxWindowError(`window token scan failed (${result.result.case})`); const { exitCode, stdout } = result.result.value; if (exitCode !== 0) return []; return [...new Set(String(stdout ?? "").split(/\s+/).filter(Boolean).map((value) => Number(value)).filter((value) => Number.isInteger(value) && value > 0))].sort((a, b) => a - b); }
