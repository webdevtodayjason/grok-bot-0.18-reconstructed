...
import { SAND_SUBAGENT_ID_PREFIX } from "../../shared/agents/subagents.js";
...
export class SandSubagentHostAdapter {
  ...
  /** SUBAGENT-1. A subagent that never launched still built a runner and can still hold a store; releasing it has to give that back. */
  releaseSession(agentId: string): void { if (this.dispatcher.isRunning(agentId)) return; const session = this.sessions.get(agentId); this.sessions.delete(agentId); const releasing = session?.dispose?.(); if (releasing != null) void releasing.catch(() => {}); this.dispatcher.freeComputerUseWindow(agentId); }
}
