...
const objectSchema = z.object({
  ...
  secret: z.object({
    ...
    connector: z.string().trim().min(1).describe('Where the value has to land. Use the reserved name "shell" when the credential is for a COMMAND you run in your own box shell -- the value is set as an environment variable of that shell and every command you run from then on sees it. The value never enters this conversation, but on this one route your own commands CAN read it back, so do not tell the user it is hidden from you. Otherwise name the connector/platform: a local stdio connector gets it in that server\'s process environment, and a chat platform (slack, github) gets it in that channel\'s credential store.'),
  ...
  }).optional().describe('Required when type is secret-request. Asks the user for a credential through a masked secure input; the value goes straight to its destination and never enters the chat or your context. You only learn that it was provided and where it landed. The one exception is connector "shell": there the destination IS your own shell environment, so your commands can read the value afterwards. Use connector "shell" with an UPPERCASE field to get a token you need for a shell command as an environment variable of your own box.'),
});
