...
export class SandAgentDb {
  ...
  appendTranscriptEntry(entry: TranscriptEntry): boolean { entry = withEvidence(this.agentDirName, entry); let inserted = false; const committed = this.runWrite("appendTranscriptEntry", () => { inserted = Number((this.statements.insertTranscriptEntry!.run(entry.id, JSON.stringify(entry)) as { changes?: unknown }).changes) > 0; }); if (committed && inserted) publishTranscriptMutation({ kind: "entries-upserted", agentId: this.agentDirName, entries: [entry] }); return committed && inserted; }
