...
import { withEvidence } from "../evidence/evidence-registry.js";
import { withJevChip } from "../../jev/chip.js";
import { withJevSources } from "../../jev/sources.js";
...
export class SandAgentDb {
  ...
  appendTranscriptEntry(entry: TranscriptEntry): boolean { entry = withJevSources(this.agentDirName, withJevChip(this.agentDirName, withEvidence(this.agentDirName, entry))); let inserted = false; const committed = this.runWrite("appendTranscriptEntry", () => { inserted = Number((this.statements.insertTranscriptEntry!.run(entry.id, JSON.stringify(entry)) as { changes?: unknown }).changes) > 0; }); if (committed && inserted) publishTranscriptMutation({ kind: "entries-upserted", agentId: this.agentDirName, entries: [entry] }); return committed && inserted; }
