...
export function ensureProfileFile(dbPath:string,db:{get(key:string):unknown;getSandProfile?():{description?:string}}):string{const path=getSandProfilePath(dirname(dbPath));if(existsSync(path))return path;writeSandProfileFile(path,{name:String(db.get("name")||SAND_DEFAULT_AGENT_NAME).trim()||SAND_DEFAULT_AGENT_NAME,description:db.getSandProfile?.().description?.trim()??"",title:"",avatarShape:"",avatarColor:""});return path}
