export const SAND_INFERENCE_PROVIDERS = ["cursor", "claude-code", "codex", "openrouter", "openai-compatible"] as const;
...
export function emptySandInferenceRouterUsage(): SandInferenceRouterUsage {
  ...
  return { schemaVersion: 1, providers: { cursor: empty(), "claude-code": empty(), codex: empty(), openrouter: empty(), "openai-compatible": empty() } };
}
