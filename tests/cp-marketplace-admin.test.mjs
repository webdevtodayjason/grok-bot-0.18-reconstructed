...
test("the console has a seventh panel and says so, and its CSS came with it", async () => {
  await withPlane(async (plane) => {
    ...
    // this saying six while the screen showed seven, and the gate would pass on a blank table.
    assert.ok(script.text.includes("window.__adminLive = { panels: 7"), "the live flag still says six panels");
