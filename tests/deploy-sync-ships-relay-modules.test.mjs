...
// BROWSER-1. Titan's browser driver is the second thing that rides the runtime mount into every
// box, beside the host bundle and the exec daemon. It is plain .mjs out of the repo with no build
// step, so the only way it reaches a box is this ship -- and the failure of forgetting it is not a
// build error, it is browser_open answering with an import failure on a live instance.
//
// Pinned the way the relay's modules are: shipped as a DIRECTORY so a file added later travels
// without anyone editing the script, guarded in install.sh so a stale tree is refused before the
// pull rather than after it, and every relative import inside the directory checked to exist,
// because the box installs no packages and cannot resolve anything the directory does not carry.
test("sync.sh ships runtime/browser-driver into the runtime mount, and install.sh refuses a tree without it", () => {
  const script = readFileSync(path.join(repo, "deploy/r750/sync.sh"), "utf8");
  const install = readFileSync(path.join(repo, "deploy/r750/install.sh"), "utf8");

  assert.match(script, /rsync -a --delete "\$REPO\/runtime\/browser-driver\/" "\$HOST:\$ROOT\/runtime\/browser-driver\/"/,
    "the driver ships as a directory, so a file added to it later travels without a second edit here");
  assert.match(script, /\[ -d "\$REPO\/runtime\/browser-driver" \]/,
    "a missing driver directory has to fail with a sentence, not with an rsync error");
  assert.match(install, /runtime\/browser-driver\/page-text\.mjs/,
    "install.sh refuses a tree with no driver in it, the way it already refuses one with no exec daemon");

  // Everything in the directory, and everything it imports of its own. The box has no npm.
  const dir = path.join(repo, "runtime/browser-driver");
  const files = readdirSync(dir).filter((name) => name.endsWith(".mjs"));
  assert.ok(files.length > 0, "runtime/browser-driver carries no modules at all");
  assert.ok(files.includes("page-text.mjs"), "page-text.mjs is the file install.sh guards on");
  for (const file of files) {
    const body = readFileSync(path.join(dir, file), "utf8");
    for (const [, specifier] of body.matchAll(/(?:^|\s)(?:import|export)[^;]*?from\s+"([^"]+)"/g)) {
      if (specifier.startsWith(".")) {
        assert.ok(files.includes(path.posix.basename(specifier)),
          `${file} imports ${specifier}, which is not in runtime/browser-driver -- the box would fail to resolve it`);
        continue;
      }
      // node: builtins are fine. A bare package name is not: nothing installs it inside a box.
      // playwright-core is the one exception the box image already carries, at
      // /usr/local/lib/node_modules, and it is reached with a createRequire, never a bare import.
      assert.ok(specifier.startsWith("node:"),
        `${file} imports the package "${specifier}"; the box installs no packages, so it has to be vendored or required from the box's own global path`);
    }
  }
});
